...
*Proof.*
In one direction, note that having a square root in \\(\mathbb Q(\zeta_N)\\) is preserved by taking products, so it is enough to show that each element of \\(S\\) has a square root in \\(\mathbb Q(\zeta_N)\\). For odd primes \\(p\\), this is Corollary 3. For \\(2\\), this is Remark 5. Lastly, for \\(-1\\), this is just noting that \\(\zeta_4=i\\).
