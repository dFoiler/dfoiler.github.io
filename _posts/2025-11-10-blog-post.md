...
This result can be neatly encoded into the quadratic Dirichlet character attached to \\(K\\). Recall that the Kronecker–Weber theorem tells us that \\(\mathbb Q^{\mathrm{ab}}\\) is a union of cyclotomic fields, so \\(\operatorname{Gal}(\mathbb Q^{\mathrm{ab}}/\mathbb Q)\\) is isomorphic to \\(\widehat{\mathbb Z}^\times\\).
...
\\[\frac1{2+1}\left|\left(\frac{-3}1\right)\right|=\frac13.\\]